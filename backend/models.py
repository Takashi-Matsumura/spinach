...
class UserInfo(BaseModel):
    ...
    department: Optional[str] = Field(None, description="User's department (部署)")
    position: Optional[str] = Field(None, description="User's position/role (役職)")


class BusinessInfo(BaseModel):
    """Business information extracted from conversation."""
    main_tasks: List[str] = Field(default_factory=list, description="Main tasks/responsibilities")
    tools_used: List[str] = Field(default_factory=list, description="Tools and systems used")


class SessionMetadata(BaseModel):
    """Structured metadata for a session."""
    user_info: UserInfo = Field(default_factory=UserInfo, description="User information")
    business_info: BusinessInfo = Field(default_factory=BusinessInfo, description="Business information")
