...
- **音声入力**:
  - Web Speech APIを使用した音声認識（対応ブラウザのみ）
  - マイクデバイス選択と切り替え
  - リアルタイム音量レベル表示（RMS計算による正確な測定）
  - マイクテスト機能
  - メッセージ送信後は自動的に音声入力モードがOFFに
- **Mermaid図表**: チャット内でのMermaid図表のレンダリング
- **スムーズなUI**:
  - ストリーミング応答時のちらつき防止（スロットル処理）
  - スムーズスクロール実装
...
- **マイク設定タブ**:
  - マイク許可の確認と要求
  - マイクデバイスの選択（複数のマイクがある場合）
  - マイクテスト: リアルタイム音量レベル表示で動作確認
  - 音量レベルはRMS計算による正確な測定

### 音声入力の使用

1. メイン画面のマイクアイコンをクリックして音声入力を開始
2. マイクに向かって話す
3. 認識されたテキストが入力欄に表示される
4. **タッチデバイス**: 指を離すと自動的にメッセージが送信される
5. **PC**: マイクアイコンをクリックで停止、送信ボタンで送信
6. メッセージ送信後、音声入力モードは自動的にOFFになる

**マイク設定**:
- アプリ情報 > マイク設定タブでマイクデバイスを選択・テストできます
- Bluetoothマイクを使用する場合は、事前にペアリングしてから選択してください
...
│   │   ├── MicrophoneSettings.tsx # マイク設定画面
│   │   └── SessionSidebar.tsx # セッション履歴
│   ├── horenso/              # 日報（報連相）機能
│   │   └── templates.ts      # 日報テンプレート定義
│   ├── hooks/                # カスタムフック
│   │   ├── useMicrophoneDevices.ts # マイクデバイス管理
